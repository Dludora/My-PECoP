...
MTL_train_path = os.path.join(
    dir_path, "datasets/MTL-AQA_split_0_data/train_split_0.pkl"
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/home/ksr/PECoP/model_rgb.pth",
    ...
    parser.add_argument(
        "--clip_len", type=int, default=64, help="64, input clip length"
    )
    ...
    parser.add_argument(
        "--model", type=str, default="i3d", help="s3d/r21d/r3d/c3d, pretrain model"
    )
    parser.add_argument(
        "--data_list",
        type=str,
    ...
    parser.add_argument(
        "--rgb_prefix",
        type=str,
        default="/home/amir/AQA/Datasets/PD/30_subjects_frames/frames/",
        help="frames",
    )
    # ksr
    parser.add_argument(
        "--data_root",
        type=str,
        default="/home/zkl/Documents/Data/AQA/Sequence/MTL-AQA/",
    )
...
def train(args):
    ...
    exp_name = "{}_sr_{}_{}_lr_{}_len_{}_sz_{}".format(
        args.dataset, args.max_sr, args.model, args.lr, args.clip_len, args.crop_sz
    )

    print(exp_name)

    pretrain_cks_path = os.path.join("pretrain_weights", exp_name)
    ...
    train_dataset = MTL_Dataset(
        args, transforms_=train_trans, color_jitter_=color_jitter
    )
...
def model_saver(net, optimizer, epoch, max_to_keep, model_save_path):
    ...
    torch.save(
        net.state_dict(),
        os.path.join(
            model_save_path, "gait_adapter" + "{:02}".format(epoch + 1) + ".pth"
        ),
    )
