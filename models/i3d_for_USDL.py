        ...
        if loRa and loRa_rank > 0:
            self.loRa_A = nn.Parameter(
                self.conv3d.weight.new_zeros(
                    (loRa_rank * kernel_shape[0], in_channels * kernel_shape[1])
                )
            )
            self.loRa_B = nn.Parameter(
                self.conv3d.weight.new_zeros(
                    (
                        output_channels
                        // self.conv3d.groups
                        * kernel_shape[0] ** (self.conv3d.weight.dim() - 3),
                        loRa_rank * kernel_shape[0],
                    )
                )
            )
...
class InceptionModule(nn.Module):
    def __init__(self, in_channels, out_channels, name, loRa=False):
...
class InceptionI3d(nn.Module):
    ...
    def __init__(
        ...
        self.end_points[end_point] = InceptionModule(
            192, [64, 96, 128, 16, 32, 32], name + end_point, loRa=True
        )
        if self._final_endpoint == end_point:
            return
        ...
        end_point = "Mixed_3c"
        self.end_points[end_point] = InceptionModule(
            256, [128, 128, 192, 32, 96, 64], name + end_point, loRa=True
        )
        if self._final_endpoint == end_point:
            return
        ...
        end_point = "Mixed_4b"
        self.end_points[end_point] = InceptionModule(
            128 + 192 + 96 + 64, [192, 96, 208, 16, 48, 64], name + end_point, loRa=True
        )
        if self._final_endpoint == end_point:
            return
    ...
    def fix_i3d(self):
        ...
        for end_point in self.GRADS:
            if end_point in self.end_points:
                self._modules[end_point].unf_lora()
